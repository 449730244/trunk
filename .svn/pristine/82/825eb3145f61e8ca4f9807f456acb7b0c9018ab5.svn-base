    ...
    function initMessageQueue()
    {
        json_get('/getQueueList',{},function(data){
            ...
            $.each(data,function(key,val){
                str += '<div class="outer" id="outer_'+ val.type +'_'+ val.from_id +'" data-type="'+ val.type +'" data-id ="'+ val.from_id +'"><div class="people_out outer"><div class="peoples persons fl" data-type="'+ val.type +'" data-id ="'+ val.from_id +'">';
                if(val.content)
                {
                    ...
                    '<p class="times" id="times_'+ val.from_id +'">'+ val.show_time+'</p>';
                if(val.unread > 0)
                {
                    if(val.unread > 99)
                    {
...
function messageShow(id,type,data)
{
    ...
    $('#outer_'+ type +'_'+ id).find('#times_'+ id).html(data.show_time);
    ...
    if(type == 'user')
    {
        ...
        if(data.content.tips == 1){
            ...
            {
                //文件名称过长缩短
               var file_name   =  subFilename(data.content.file_name)
                msg_str += '<div class="file_style" title="'+data.content.file_name+'">' +
                    '         <div class="file_menu">' +
...
function messageShowToUser(data)
{
    ...
    if($('#outer_'+ data.type +'_'+ data.user_id).length == 0)
    {
        ...
        $('#outer_'+ data.type +'_'+ data.user_id).find('#times_'+ data.user_id).html(data.show_time);
...
function messageShowToGroup(data)
{
    ...
    if($('#outer_'+ data.type +'_'+ data.group_id).length == 0)
    {
       ...
       // }
        ...
        $('#outer_'+ data.type +'_'+ data.group_id).find('#times_'+ data.group_id).html(data.show_time);
...
function queueShow(data){
    ...
    if(data.type == 'user')
    {
        str += '<div class="outer" id="outer_'+ data.type +'_'+ data.user_id +'"><div class="people_out outer"><div class="peoples persons fl" data-type="'+ data.type +'" data-id ="'+ data.user_id +'">';
        if(data.content)
        {
            if(data.type == 'group')
            {
        ...
        str += '<div class="outer" id="outer_'+ data.type +'_'+ data.group_id +'"><div class="people_out outer"><div class="peoples persons fl" data-type="'+ data.type +'" data-id ="'+ data.group_id +'">';
        if(data.content)
        {
            str += '<div class="circle fl"><p class="radius"><img src="/chat/img/grouphead.png" alt=""></p></div>';
...
function contentShow(data)
{
    ...
    if(data.content.file_id)
    {
        var filename    =   subFilename(data.content.file_name)
        msg_str += '<div class="file_style" title="'+data.content.file_name+'">' +
            '         <div class="file_menu">' +
            '           <span class="down" data-fileid="'+ data.content.file_id +'"></span>' +
            '         </div>' +
...
$(document).ready(function () {
    ...
    $('#testdiv').keydown(function (e) {
        ...
        $('body').delegate('.persons','click',function (){
        $('.conversation').empty();
        //获取当前点击的聊天类型和id
        console.log('userid----------',$("#chat_top").data('userid'));
        var  src = $(this).find('img').attr('src');
            $('#chat_top .chart_r .head_img').html('<img src="'+ src +'"/>');
            $('#chat_top .chart_r .username').text($(this).find('.tit').text());
